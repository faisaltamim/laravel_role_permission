             ...
             <div class="col-6 mt-5">
                 <div class="card">
                     <div class="card-body">
                         ...
                         <form action="{{ route('admin.users.store') }}" id="UserSubmissionForm" method="POST">
                             @csrf
                             ...
                             <div class="form-group">
                                 ...
                                 <input type="email" class="form-control  @error('email') is-invalid @enderror "
                                     id="email" name="email" placeholder="User Email" value="{{ old('email') }}">
                                 @error('email')
                                     <span class="fw-bold text-danger">{{ $message }}</span>
                                 @enderror
                             </div>
                             ...
                             <div class="form-group @error('roles') customErrorCss @enderror">
                                 ...
                                 <select name="roles[]" id="roles" multiple="multiple" class="3col active"  >
                                     @foreach ($roles as $role)
                                         <option value="{{ $role->name }}" {{ old('password')? 'selected':'' }}>{{ $role->name }}</option>
                                     @endforeach
                                 </select>
                                 @error('roles')
                                     <span class="fw-bold text-danger">{{ $message }}</span>
                             ...
                             <button type="submit" class="btn btn-sm btn-success mt-4 pr-4 pl-4">Add User</button>
                         </form>
                     </div>
